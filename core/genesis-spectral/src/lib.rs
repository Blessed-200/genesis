...
mod tests {
    ...
    fn dirac_on_flat_manifold_zero_curvature() {
        ...
        let ricci = dirac.squared().ricci_scalar;
        assert!(ricci.is_finite());
        assert!((ricci + 8.0).abs() < 1e-12, "flat manifold residual curvature {ricci}");
    }
    ...
    fn dirac_squared_on_flat_equals_double_apply() {
        ...
        let via_matrix = d.squared().matrix.matvec(&v);
        let via_double_apply = d.apply(&d.apply(&v));
        for i in 0..16 {
            assert!((via_matrix[i] - via_double_apply[i]).abs() < 1e-12, "D² mismatch at index {i}");
        }
    }
    ...
    fn spectral_action_monotone_decrease() {
        ...
        let mut engine = SpectralFlowEngine::new(1.0, 0.1, CutoffFunction::Gaussian, ConvergenceCriteria { grad_tol: 1e-10, max_steps: 100 });
        for _ in 0..100 {
    ...
    fn spectral_distance_recovers_grade_metric() {
        let mut b = [0.0; 16];
        b[1] = 1.0;
        let d = DiracOperator::from_blades(&b, 1.0).expect("dirac").commutator_norm(&[1.0; 16]);
        assert!(d.is_finite());
        assert!(d >= 0.0);
    }
