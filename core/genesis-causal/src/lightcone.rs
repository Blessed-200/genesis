...
impl<'a> LightconeFilter<'a> {
    #[must_use]
    pub fn new(causal_order: &'a CausalOrder) -> Self { Self { causal_order } }
    ...
    pub fn validate_inference(&self, inference: &CausalInference) -> Result<(), GenesisError> {
        ...
        for &premise in &inference.premise_ids {
            if !past.contains(&premise) {
                return Err(GenesisError::AcausalInference { premise_id: premise, conclusion_id: inference.conclusion_id });
            }
        }
        Ok(())
    }

    pub fn explain_inference_failure(&self, inference: &CausalInference) -> Result<(), GenesisError> {
        self.validate_inference(inference)
    }

    #[must_use]
    pub fn is_before(&self, a: u64, b: u64) -> bool { self.causal_order.is_before(a, b) }

    #[must_use]
    pub fn ancestors_of(&self, node: u64) -> SmallVec<[u64; 16]> { self.causal_order.ancestors_of(node) }

    #[must_use]
    pub fn descendants_of(&self, node: u64) -> SmallVec<[u64; 16]> { self.causal_order.descendants_of(node) }

    #[must_use]
    pub fn cone_overlap(&self, a: u64, b: u64) -> SmallVec<[u64; 16]> { self.causal_order.cone_overlap(a, b) }
}
