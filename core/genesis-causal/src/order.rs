...
impl CausalEdge {
    pub fn compute(
        ...
        let dot = cause_blades[1] * effect_blades[1]
            + cause_blades[2] * effect_blades[2]
            + cause_blades[4] * effect_blades[4]
            + cause_blades[8] * effect_blades[8];
        let causal_strength = dot.abs().min(1.0);
...
impl CausalOrder {
    #[must_use]
    pub fn new() -> Self {
        Self { edges: Vec::new(), causal_frontier: SmallVec::new() }
    }
    ...
    pub fn past_lightcone(&self, node_id: u64) -> SmallVec<[u64; 16]> {
        ...
        while let Some(current) = queue.pop() {
            for edge in &self.edges {
                if edge.effect_id == current && !visited.contains(&edge.cause_id) {
                    visited.push(edge.cause_id);
                    queue.push(edge.cause_id);
                }
            }
        }
    ...
    pub fn verify_acyclic(&self) -> Result<(), GenesisError> {
        ...
        for e in &self.edges {
            if !all_nodes.contains(&e.cause_id) { all_nodes.push(e.cause_id); }
            if !all_nodes.contains(&e.effect_id) { all_nodes.push(e.effect_id); }
        }
        let mut in_degree: SmallVec<[(u64, usize); 64]> = all_nodes.iter().map(|&id| (id, 0)).collect();
        for e in &self.edges {
            if let Some(entry) = in_degree.iter_mut().find(|(id, _)| *id == e.effect_id) { entry.1 += 1; }
        }
        let mut queue: SmallVec<[u64; 16]> = in_degree.iter().filter(|(_, d)| *d == 0).map(|(id, _)| *id).collect();
        let mut visited = 0usize;
        while let Some(node) = queue.pop() {
            visited += 1;
            let start = self.edges.partition_point(|e| e.cause_id < node);
            for edge in &self.edges[start..] {
                if edge.cause_id != node { break; }
                if let Some(entry) = in_degree.iter_mut().find(|(id, _)| *id == edge.effect_id) {
                    entry.1 = entry.1.saturating_sub(1);
                    if entry.1 == 0 { queue.push(entry.0); }
                }
            }
        }
        if visited < all_nodes.len() { Err(GenesisError::CausalCycle { cycle_nodes: vec![] }) } else { Ok(()) }
    }
    ...
    fn update_frontier(&mut self) {
        ...
        self.causal_frontier = all_causes.iter().filter(|id| !effects.contains(id)).copied().collect();
    }
}
