...
- Allocation impact:
  ...
  - Triangle scratch avoids heap allocation until intersections exceed inline capacity.
- `cargo bench -p genesis-topology --bench topology -- xor_row_elimination_throughput`
- `cargo bench -p genesis-topology --bench topology -- rips_build_h1_hotpath`
- `cargo bench -p genesis-topology --bench iai_hotpaths --no-run`
- `perf stat -d target/release/deps/topology-* --bench` (capture IPC, LLC misses, branch-miss rate)
- `valgrind --tool=cachegrind target/release/deps/topology-* --bench` (cache-line pressure for cohomology/rips hot paths)
- `hyperfine --warmup 3 'cargo bench -p genesis-topology --bench topology -- xor_row_elimination_throughput'`

Performance evidence checklist:
- Complexity before/after:
  - Z2 row elimination remains `O(rows * cols/64)` but removes repeated feature-detection in the per-row hot loop.
  - Rips lookup path keeps `O(E log V)` worst case; consecutive-ID fast path replaces lookup with `O(1)` direct index access.
- Expected cache behavior:
  - XOR kernel selection occurs once per elimination call; row tails execute branch-stable SIMD/scalar loops over contiguous packed words.
  - Triangle scratch uses `SmallVec<[usize;64]>` to keep typical neighbor intersections stack-resident and avoid heap churn.
- Allocation impact:
  - Elimination hot path: no new heap allocation in XOR application loop.
  - Triangle intersection hot path: scratch allocation is stack-first; heap spill only beyond 64 common neighbors.
